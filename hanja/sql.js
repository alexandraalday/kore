...
module.exports = class Hanja {
    ...
    async searchWords(args) {
        ...
        const similarwords = await this.sequelize
            .query(`select hanja, hangul, english
                    from (select hanja, hangul, english
                          from hanjas
                          where hidden_index match ${query}
                          union
                          select hanja, hangul, english
                          from hanjas
                          where english match ${query}
                          union
                          select hanja, hangul, english
                          from hanjas
                          where hangul match ${query}
                          union
                          select hanja, hangul, english
                          from hanjas
                          where hanjas match ${query})
                    order by hangul`, {
        ...
        await asyncForEach(characters, async (character) => {
            const hanjas = await this.sequelize
                .query(`select hanjas, definition
